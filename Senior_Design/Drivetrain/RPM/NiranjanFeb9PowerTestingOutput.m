...
% Interval to average RPM data over (average last 25 activations)
interval = 25;
...
% averaged RPM value)
temp1Times = zeros(interval,1);
temp2Times = zeros(interval,1);
temp3Times = zeros(interval,1);
...
while toc <= duration
    ...
    if serialConnect.NumBytesAvailable > 0
        ...
        if data == 1
            temp1Times = [temp1Times(2:interval) currentTime];
        elseif data == 2
            temp2Times = [temp2Times(2:interval) currentTime];
        elseif data == 3
            temp3Times = [temp3Times(2:interval) currentTime];
        end

        % Calculate RPM values 
        RPM1 = 60.0 / mean(nonzeros(diff(temp1Times)));
        RPM2 = 60.0 / mean(nonzeros(diff(temp2Times)));
        RPM3 = 60.0 / mean(nonzeros(diff(temp3Times)));

        % Update plot
        addpoints(r1, currentTime, RPM1);
        addpoints(r2, currentTime, RPM1);
        addpoints(r3, currentTime, RPM1);
